...
(function () {
  ...
  /**
   * Normalize image paths to include ../ prefix for CSS relative path resolution
   * When using paths in CSS from html files, they need to account for CSS being in css/ subfolder
   */
  function normalizePath(imagePath) {
    if (!imagePath) return imagePath;
    // If path starts with 'data/', add '../' prefix for correct relative resolution
  ...
  function preloadImage(src) {
    return new Promise((resolve, reject) => {
      ...
      if (preloadedImages.has(normalizedSrc)) {
        const cached = preloadedImages.get(normalizedSrc);
      ...
      const img = new Image();
      img.onload = () => {
        preloadedImages.set(normalizedSrc, img);
        resolve(img);
      };
      img.onerror = () => {
        preloadedImages.set(normalizedSrc, null); // Mark as failed
        resolve(null);
      };
  ...
  async function fetchHeritagePhotos() {
    try {
      if (
        ...
        typeof MatiSupabaseApi.listMedia !== "function"
      ) {
        setDefaultImages();
  ...
  function getRandomPhotos() {
    ...
    const intangiblePhoto =
      workingIntangible.length > 0
        ? workingIntangible[
            Math.floor(Math.random() * workingIntangible.length)
          ]
        : DEFAULT_INTANGIBLE_IMAGES[
            Math.floor(Math.random() * DEFAULT_INTANGIBLE_IMAGES.length)
          ];

    const naturalPhoto =
      workingNatural.length > 0
        ? workingNatural[Math.floor(Math.random() * workingNatural.length)]
        : DEFAULT_NATURAL_IMAGES[
            Math.floor(Math.random() * DEFAULT_NATURAL_IMAGES.length)
          ];
  ...
  function updateBackground() {
    ...
    Promise.all([
      ...
      // Normalize paths for correct CSS resolution
      const normalizedIntangible = normalizePath(intangiblePhoto);
      const normalizedNatural = normalizePath(naturalPhoto);

      // Update the global stage background (for desktop/tablet)
      categoryStage.style.setProperty(
        "--gal-bg-intangible",
        `url("${normalizedIntangible}")`,
      );
      categoryStage.style.setProperty(
        "--gal-bg-natural",
        `url("${normalizedNatural}")`,
      );

      // On mobile portrait, also update individual card backgrounds
      if (isMobilePortrait) {
        const intangibleCard = document.querySelector(
          ".gal-category-card--intangible",
        );
        const naturalCard = document.querySelector(
          ".gal-category-card--natural",
        );

        if (intangibleCard) {
          intangibleCard.style.setProperty(
            "--gal-card-bg-intangible",
            `url("${normalizedIntangible}")`,
          );
        }
        if (naturalCard) {
          naturalCard.style.setProperty(
            "--gal-card-bg-natural",
            `url("${normalizedNatural}")`,
          );
        }
